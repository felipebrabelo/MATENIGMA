    ...
    while (!WindowShouldClose() && !*gameover_win && !*gameover_lose) {
        ...
        if (acertos >= pares_quantidade &&
            ...
            *gameover_lose = true;
            ...
            DrawTextB(erro_string, 20 + erros.width, 115, 20, WHITE);
            DrawTextB(fase_nome, screenWidth - offset_fase.x, 10, 20, WHITE);
            if (tipo_anim == 0) {
...
void TelaCompleta(double pontos_total) {
    ...
    while (!WindowShouldClose() && !termina) {
        ...
        DrawTextB("VITORIA!", (screenWidth - offset_msg.x) / 2, 10, 40, WHITE);
        DrawTextB(pontos_string, (screenWidth - offset_pontos.x) / 2, offset_msg.y + 10, 30, WHITE);
        if (relogio.atual > 5.0) {
            DrawTextB("Pressione ENTER para continuar.", (screenWidth - offset_enter.x) / 2,
